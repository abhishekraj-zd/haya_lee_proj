...
def load_merge(county):
    tax = pd.read_csv(f"complete/{county}_TAX_test.csv", converters={'parcel_ID': str})
    sdat = pd.read_csv(f"complete/{county}_SDAT_test.csv", converters={'account': str})
    return tax, sdat
...
if __name__ == '__main__':
    ...
    #
    county_dict = { 0 : 'Anne Arundel County', 1 : 'Montgomery County', 2 : "Prince George's County"}
    # for i in range(3):
    arg_county = 0
    if arg_county == 0:
        county = "Anne Arundel"
    ...
    tax, sdat = load_merge(county_dict[arg_county])
    print(tax)
    ...
    if arg_county == 0:
        final_columns = [e for e in total_columns if e not in ('block','tax_period', 'lot', 'sub', 'occupancy', 'mortgage', 'prop_address', 'detail_address', 'tax_sale', 'class_id', 'attorney_name', 'attorney_phone', 'purchaser_name', 'equity_case_no', 'bid_amount', 'base', 'ip', 'amount')] # class_id
    elif arg_county == 1:
        final_columns = [e for e in total_columns if e not in ('tax_month','tax_sale',  'prop_address', 'detail_address', 'occupancy', 'account', 'attorney_name', 'attorney_phone', 'purchaser_name', 'equity_case_no', 'bid_amount', 'base', 'ip', 'amount')]
    elif arg_county == 2:
        final_columns = [e for e in total_columns if e not in ('tax_period', 'lot', 'sub', 'class_id', 'occupancy', 'mortgage', 'prop_address', 'account', 'detail_address')]
    # print(final_columns)
